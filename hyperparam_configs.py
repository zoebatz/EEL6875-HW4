...
'''# create all combos
param_grid = list(itertools.product(chunk_sizes, learning_rates, batch_sizes, total_timesteps))
...
for i, (chunk, lr, batch, steps) in enumerate(param_grid):
    print(f"\n=== Run {i+1}/{len(param_grid)} ===")
    print(f"chunk={chunk}, lr={lr}, batch={batch}, steps={steps}")

    # Unique folder name for this configuration
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    run_name = f"chunk{chunk}_lr{lr}_batch{batch}_steps{steps}"
